...
#define STATINC_W(name) do { stats.name++; stats.name##_weighed += weight; } while (0)
	...
	static void getStoredTypesCallInst(TodoList<Value> &todo, CallInst *ci, Value *value) {
		...
		for (unsigned i = 0; i < ci->getNumArgOperands(); i++) {
			...
			unsigned index = 0;
			for (Argument &argFunc : calledFunction->getArgumentList()) {
				if (index == 0) {
					todo.add(&argFunc);
					break;
				}
	...
	bool runOnMallocCall(
		...
		size_t argcount = callee->getArgumentList().size();
		...
		CallInst *callnew = CallInst::Create(calleenew, argsnew, "", ci);
	...
	CallSiteType getCallSiteType(CallInst *ci) {
		Function *callee = ci->getCalledFunction();
	...
	uint64_t getDebugLocHash(CallInst *ci, unsigned callIndex, bool log) {
	...
	bool runOnCallInstruction(CallInst *ci,
		unsigned &callindex,
		struct statistics &stats,
		const std::map<uint64_t, long> &callsiteInlineCounts) {
